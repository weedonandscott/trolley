...
pub fn build_formats(
    ...
    for &planned in planned_formats {
        let (name, result) = match planned.format() {
            ...
            Format::Rpm => {
                ...
                let result =
                    rpm::build(project_dir, bundle_dir, dist_dir, config, manifest, planned);
                progress.finish_and_clear();
                ("RPM", result)
            }
            Format::Deb => {
                let progress = spinner("Building deb...");
                let result = packager_common::run_packager(
                    config,
                    project_dir,
                    bundle_dir,
                    dist_dir,
                    manifest,
                    &[packager_common::PackagerFormat::Deb],
                );
                progress.finish_and_clear();
                ("deb", result)
            }
            Format::AppImage => {
                ...
                let result = packager_common::run_packager(
                    ...
                    dist_dir,
                    manifest,
                    &[packager_common::PackagerFormat::AppImage],
                );
                progress.finish_and_clear();
                ("AppImage", result)
            }
            Format::Pacman => {
                ...
                let result = packager_common::run_packager(
                    ...
                    dist_dir,
                    manifest,
                    &[packager_common::PackagerFormat::Pacman],
                );
                progress.finish_and_clear();
                ("pacman", result)
            }
            Format::Nsis => {
                ...
                let result = packager_common::run_packager(
                    ...
                    dist_dir,
                    manifest,
                    &[packager_common::PackagerFormat::Nsis],
                );
                progress.finish_and_clear();
                ("NSIS", result)
            }
            Format::MacApp => {
                ...
                let result = packager_common::run_packager(
                    ...
                    dist_dir,
                    manifest,
                    &[packager_common::PackagerFormat::MacApp],
                );
                progress.finish_and_clear();
                ("app", result)
            }
            Format::Dmg => {
                ...
                let result = packager_common::run_packager(
                    ...
                    dist_dir,
                    manifest,
                    &[packager_common::PackagerFormat::Dmg],
                );
                progress.finish_and_clear();
                ("dmg", result)
            }
    ...
    if !failed.is_empty() {
        eprintln!("\n{} format(s) failed: {}", failed.len(), failed.join(", "));
    }
